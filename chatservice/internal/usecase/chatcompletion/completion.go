...
import (
	...
	"github.com/viktorhugodev/chatGPT-whatsapp/chatservice/internal/domain/gateway"
)
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	userMessage, err := entity.NewMessage("user", input.UserMessage, chat.Config.Model, []openai.ChatCompletionMessage{})
	if err != nil {
	...
	assistant, err := entity.NewMessage("assistant", resp.Choices[0].Message.Content, chat.Config.Model, []openai.ChatCompletionMessage{})
	if err != nil {
		return nil, err
	}
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	initialMessage, err := entity.NewMessage("system", input.Config.InitialSystemMessage, model, []openai.ChatCompletionMessage{})
	if err != nil {
